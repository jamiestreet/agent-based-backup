        ...
        if (selfIsolating[i] == 0){
            ...
            if (workplace_no[i] > -1){
                if (workplaces1[workplace_no[i]].size() > 1){
                    ...
                    if (LockdownBool == 1) {
                        c = household_critical[household_no[i]] * c;
                    }
                    while (b < c){
...
vector<int> household_critical_assign(int nPop, vector<vector<int>> households1, vector<int> ages){
    random_device randomDevice;
    mt19937 generator(randomDevice());
    uniform_real_distribution<double> distributionC(0, 1);
    vector<int> household_critical(households1.size());
    for (int i=0; i < households1.size(); i++){
        if (ages[households1[i][0]] > 1 && ages[households1[i][0]] < 7){
            float m = distributionC(generator);
            if (m < 0.3){
                household_critical[i] = 1;
            }
        }
    }
    return household_critical;
}
...
vector<int> check_selfIsolating(int nPop, vector<int> states, vector<int> infTypes, vector<int> selfIsolating){
    for (int i=0; i < nPop; i++){
        if (infTypes[i] == 2 && states[i] == 2){
            selfIsolating[i] = 1;
        }
        if (states[i] == 3){
            selfIsolating[i] = 0;
...
int main() {
    ...
    int SocialDay = 30;
    int SocialEnd = 120;
    float SocialDEffect = 0.18;
    int LockdownDay = 60;
    float LockdownEffect = 0.1;
    int LockdownEnd = 90;
    int LockdownBool = 0;
    ...
    vector<int> household_critical = household_critical_assign(nPop, households1, ages);
    ...
    int selfIsolatingDay = -1;
    int selfIsolatingFact = 0;
    int testingDay = -1;
    float testingProb = 0.5;
    int testingFact = 0;
    vector<int> newInfections;
    while(time < days){
        if (time == SocialDay){
            TransmProb = TransmProb * SocialDEffect;
        }
        if (time == LockdownDay){
            meetAve = ceil(meetAve * LockdownEffect);
            LockdownBool = 1;
        }
        if (time == selfIsolatingDay){
            selfIsolatingFact = 1;
        }
        if (time == testingDay){
            testingFact = 1;
        ...
        if (selfIsolatingFact == 1){
            selfIsolating = check_selfIsolating(nPop, states, infTypes, selfIsolating);
        }
        if (testingFact == 1){
            selfIsolating = random_testing(nPop, states, selfIsolating, infTypes, testingProb);
        }
        vector<int> new_infs = iteration(nPop, TransmProb, meetAve, work_meetAve, states, infTypes, households1, household_no, workplaces1, workplace_no, selfIsolating, LockdownBool, household_critical, TransmProbOrg);
        int o2 = 0;
