...
class abmHouseholds:
    ...
    def iteration(self):
        ...
        for i in range(self.nPop):
            ...
            if self.states[i] == 1 and self.latencyLen[i] == self.latencyTime[i]:
                ...
                self.infectiousLen[i] = 0
                self.latencyLen[i] = -1
                if self.infectionType[i] == 2 and self.isolationRule:
            ...
            if self.isolating[i] == 0:
                ...
                if self.isolating[i] == 0:
                    ...
                    for j in range(len(meetings)):
                        ...
                        if self.states[meetings[j]] == 2 and self.states[i] == 0 and self.isolating[meetings[j]] == 0:
    ...
    def summary(self):
        ...
        e = [self.states[x] for x in range(self.nPop) if self.states[x] == 2 and self.infectionType[x] == 1]
        f = [self.states[x] for x in range(self.nPop) if self.states[x] == 2 and self.infectionType[x] == 2]
...
x = [cos( angle * i ) for i in range(nPop)]
y = [sin( angle * i ) for i in range(nPop)]
...
def pointsGen(lis):
    ...
    for i in range(len(lis)-1):
        for j in range(i+1, len(lis)):
            out.append([lis[i], lis[j]])
    return out

cols = ['b', 'g', 'r', 'c', 'm', 'y', 'k']

def plotLines(x1, y1, col, alp):
    x2 = [[cos(angle * x1[i]), cos(angle * y1[i])] for i in range(len(x1))]
    y2 = [[sin(angle * x1[i]), sin(angle * y1[i])] for i in range(len(y1))]

    for i in range(len(x2)):
        plt.plot(x2[i], y2[i], color=col, alpha=alp)

households = model.households
points = []
for i in range(len(households)):
    points = pointsGen(households[i])
    x2 = [points[i][0] for i in range(len(points))]
    y2 = [points[i][1] for i in range(len(points))]
    plotLines(x2, y2, cols[i%7], 0.5)

print(points)

meetings = model.all_meetings

x1 = [meetings[i][0] for i in range(len(meetings))]
y1 = [meetings[i][1] for i in range(len(meetings))]

x2 = [points[i][0] for i in range(len(points))]
y2 = [points[i][1] for i in range(len(points))]

#plotLines(x2, y2, "red", 0.5)
plt.axis('off')
plt.show()
